...
import { calculatePlannerResult } from "@/lib/calculations/engine";
import { plannerSchema, type PlannerFormInput } from "@/lib/schemas/planner";
import type { CalculationResult } from "@/lib/models";

const fixtureDefaults = [{ fixtureId: fixtures[0].id, quantity: 1 }];
...
export function CalculatorForm() {
  ...
  return (
    <section className="mx-auto max-w-4xl rounded-2xl border border-lumini-300 bg-white p-8 shadow-sm">
      ...
      <p className="mt-2 text-lumini-700">Professional low-voltage planning for trade and specification teams.</p>
      ...
      <form onSubmit={form.handleSubmit(runCalculation)} className="mt-8 space-y-8">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            {values.fixtures.map((fixture, index) => (
              <div className="grid grid-cols-2 gap-3" key={`${fixture.fixtureId}-${index}`}>
                <select className="rounded border p-2" {...form.register(`fixtures.${index}.fixtureId`)}>
                  {fixtures.map((item) => (
                    <option key={item.id} value={item.id}>{item.name} · {item.wattage}W</option>
                  ))}
                </select>
                <input
                  type="number"
            ...
            <button type="button" className="rounded border px-3 py-2" onClick={addFixture}>Add fixture</button>
          </div>
        )}
        ...
        {step === 5 && result && (
          <div className="space-y-4" data-testid="result-panel">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <button type="button" className="rounded bg-lumini-900 px-4 py-2 text-white">Email me this result</button>
              <button type="button" className="rounded border px-4 py-2">Download PDF</button>
              <button type="button" className="rounded border px-4 py-2">Request full fixture schedule</button>
              <button type="button" className="rounded border px-4 py-2">Send plans for review</button>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          {step === 4 && <button type="submit" className="rounded bg-lumini-900 px-4 py-2 text-white">Calculate</button>}
        </div>
      </form>
    </section>
  );
